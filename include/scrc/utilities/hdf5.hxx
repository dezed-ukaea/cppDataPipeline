...
namespace SCRC
{
    namespace HDF5
    {
        ...
        const std::vector<std::string> read_hdf5_as_str_vector(const std::filesystem::path file_name, const std::string key);

        template<typename T>
        const T read_hdf5_object(const std::filesystem::path file_name, std::string key)
        {
            APILogger->debug("Utilities:HDF5:ReadObject: Opening file '{0}'", file_name.string());
            
            if(!std::filesystem::exists(file_name))
            {
                throw std::runtime_error("Could not open HDF5 file '"+file_name.string()+"', file does not exist.");
            }
        ...
        template<typename T>
        const std::vector<T> read_hdf5_comp_type_member(const std::filesystem::path file_name, std::string key, std::string member)
        {
            APILogger->debug("Utilities:HDF5:ReadCompTypeMember: Opening file '{0}'", file_name.string());
            ...
            APILogger->debug("Utilities:HDF5:ReadCompTypeMember: Extracting member '" + member + "' from Compound Type");
        ...
        const CompType* get_comptype(const std::filesystem::path file_name, std::string key);
    };
};
