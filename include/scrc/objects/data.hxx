...
namespace SCRC
{
    ...
    {   
        ...
        public:
            DataTableColumn() {}
            DataTableColumn(std::string name, std::string unit, const std::vector<std::string>& row_names, const std::vector<T>& values) :
                name_(name),
                unit_(unit),
            ...
            T operator[] (const char* key)
            {
                if(row_names_.empty())
                {
                    throw std::runtime_error("Cannot access label-less data table column by label");
                }

                const auto iterator_ = std::find(row_names_.begin(), row_names_.end(), std::string(key));
                if(iterator_ == row_names_.end())
                {
                    throw std::invalid_argument("No row name '"+std::string(key)+"' in column '"+name_+"'");
                }
    ...
    class ArrayObject
    {
        ...
        public:
            ArrayObject() {}
            ArrayObject(std::vector<std::string> dim_titles, std::vector<std::vector<std::string>>& dim_names, std::vector<int>& size, std::vector<T>& values) :
                dimension_titles_(dim_titles),
                dimension_names_(dim_names),
                dimensions_(size),
                array_(values) {}
            T get(std::vector<int> co_ords) const
            {
                ...
                for(int i{0}; i < dimensions_.size(); ++i) 
                {
                    ...
                    {
                        throw std::invalid_argument(
                            "Coordinate["+std::to_string(i)+"] exceeds dimension index limit: "+std::to_string(co_ords[i])+" > "+std::to_string(dimensions_[i]-1)
                        );
                    }
            ...
            std::string get_title(unsigned int dimension) const
            {
                ...
                {
                    throw std::invalid_argument("Dimension "+std::to_string(dimension)+ " exceeds array rank "+std::to_string(rank()));
                }
