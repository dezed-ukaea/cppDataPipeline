...
const std::vector<std::string> SCRC::HDF5::read_hdf5_as_str_vector(const std::filesystem::path file_name, const std::string key)
{
    APILogger->debug("Utilities:HDF5:ReadAsStrVector: Opening file '{0}'", file_name.string());
    ...
    for(auto& c : data_) data_str_.push_back(std::string(c));
...
const CompType* SCRC::HDF5::get_comptype(const std::filesystem::path file_name, std::string key)
{
    APILogger->debug("Utilities:HDF5:GetCompType: Opening file '{0}'", file_name.string());
